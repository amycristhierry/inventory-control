...
def expected_reward(x, z):
    """通过求和计算期望奖励和下一阶段价值"""
    total = 0
    for i, d in enumerate(discrete_demand):
        # 计算下一阶段的库存水平
        next_x = x + z - d
        # 找到最接近的状态索引
        next_state_index = np.abs(state_space - next_x).argmin()
        # 计算当前需求下的奖励和下一阶段价值
        reward_value = reward(x, z, d)
        next_value = V[t + 1, next_state_index]
        # 加权求和
        total += (reward_value + next_value) * demand_pmf[i]
    return total
...
for t in range(T - 1, -1, -1):
    ...
    for i, x in enumerate(state_space):
        ...
        for z in action_space:
            # 计算期望奖励和下一阶段价值
            total_value = expected_reward(x, z)
